...
//     setLoading(true)
//     // const responce = addCart(product, quantities[product] || 1)
//     // if (responce) {
//     //   setLoading(false)
...
//                       {/* <p className='flex gap-2'>  {loading ? <p><p><CartLoader /> <p></p>Adding...</p>:productIcons.cart} Add to Cart  </p> */}
...
//                           </span>
//                         ) : (
//                           <span>Add to Cart {productIcons.cart}</span>
//                         )}
...
const ProductRoute = () => {
  const { triggerPopUp, PopUp } = usePopUp();
  const navigate = useNavigate();
  const [loadingId, setLoadingId] = useState(null); // Track loading for a specific product
  const [isloading, setIsloading] = useState(true);
  const { allProducts, addCart, quantities, updateQuantity, cart, setVisibleSearch } = useContext(ContextProvider);
  const { id } = useParams();
  ...
  const onAddCart = async (productId) => {
    ...
    const response = await addCart(productId, quantities[productId] || 1); // Simulate adding to cart
    if (response) {
      setLoadingId(null); // Stop loading once added to the cart
      triggerPopUp(true, 'Item added to cart');
    }
  };
          ...
          >
            ...
            <div className="p-4 sm:p-6 flex flex-col justify-between w-full sm:w-1/2">
              ...
              <div className="quantity flex items-center gap-2 mb-4">
                <button
                  className="bg-gray-200 hover:bg-gray-300 text-gray-700 font-bold py-2 px-4 rounded transition-colors"
                  onClick={() => updateQuantity(product._id, Math.min(10, (quantities[product._id] || 1) + 1))}
                >
                  +
                </button>
                <input
                  ...
                  >
                    {loadingId === product._id ? (
                      <span className="flex items-center gap-3">
                        <p><CartLoader /></p><p> Adding...</p>
                      </span>
                    ) : (
                      <span className='flex gap-1 items-center justify-center'>Add to Cart {productIcons.cart}</span>
                    )}
                  </button>
                )}

                <button
                  className="flex items-center justify-center py-2 px-4 text-white bg-orange-500 rounded-lg shadow-md transition-transform transform hover:bg-orange-600 capitalize"
